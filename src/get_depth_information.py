...
if __name__ == "__main__":
    ...
    while True:
        ...
        img_color_warped = cv2.warpPerspective(color_image, ct_matrix, (RESOLUTION_WIDTH, RESOLUTION_HEIGHT))
        img_warped = cv2.warpPerspective(transformed_depth_image, ct_matrix, (RESOLUTION_WIDTH, RESOLUTION_HEIGHT))

        pix_x = color_image.shape[1] // 2
        pix_y = color_image.shape[0] // 2
        rgb_depth = img_warped[pix_y, pix_x]
        if rgb_depth:
            print(rgb_depth)

        # Overlay body segmentation on depth image
        cv2.imshow('Transformed Color Image', img_color_warped)
